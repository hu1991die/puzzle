...
var d_direct = new Array(
	...
	[2,4],//大div的编号为1的div可以去的位置，比如第一块可以去2,4的位置
	[1,3,5], 
	[2,6],
	[1,5,7],
	[2,4,6,8],
...
function move(id){
	...
	//如果terget_d不为0，则表示可以移动，且target_d就是小div要去的大div的位置编号
	...
	for (var k = 1; i < 9; ++k) {
		if(d[k] != k){
			 ...
			 * 顺序排的，那么设置为false，跳出循环，后面不用再判断了，因为
			 * 只要一个不符合，就没有完成游戏
			 */
		}
	}
	...
	/**
	 * 如果为true，则表示游戏完成，如果当前没有暂停，则调用暂停函数，并且弹出
	 * 提示框，完成游戏，start()这个函数是开始，暂停一起的函数，如果暂停，调用后
	 * 会开始，如果开始，则调用后会暂停
	 */
}
...
function whereCanTo(cur_div){
	/**
	 * 判断是否可以移动函数，参数是大div的编号，不是小div的编号，因为小div的
	 * 编号跟可以去哪没关系，小div是会动的
	 */
	var j = 0;
	var move_flag = false;
...
function random_d(){
	for (var i = 9; i > 1; --i) {
		//产生随机数，范围为1到i，不能超出范围，因为没有这个id的div
